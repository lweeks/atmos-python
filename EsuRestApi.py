...
class EsuRestApi(object):
    ...
    def __sign(self, headers):
        ...
        rawhash = hmac.new(decodedkey, headers, hashlib.sha1).digest()
        hashout = base64.encodestring(rawhash).strip()
    ...
    def __parse_atmos_error(self, error):
        try:
            ...
            # We wrap this ParseError as though it were an Atmos error,
            # extracted from the XML.
            self.code = Element(tag="Code")
            self.message = Element(tag="Message")
            self.code.text = ESU_XML_PARSE_ERROR
            self.message.text = repr(error)

            print("{}: ".format(self.tag) + self.message.text)
            print('{}: {}'.format(e.__class__.__name__, repr(e)))
    ...
    def __add_client_id_header(self, request, client_id):
        if self.client_id_header_mode and client_id:
            request.add_header(HDR_X_CLIENT_REQUEST_ID, client_id)


    def __get_query_client_id(self, url, client_id, extent=None, quote=True):
        query_parameters = {}
        query_str = ''

        if not self.client_id_header_mode:
            if client_id and quote:
                query_parameters[HDR_X_CLIENT_REQUEST_ID] = urllib.quote(client_id)
            elif client_id and not quote:
                query_parameters[HDR_X_CLIENT_REQUEST_ID] = client_id

        if extent:
            query_parameters['byte-range'] = extent

        query_str = "?{}".format(
            "&".join(['{}={}'.format(key, value) for key, value in query_parameters.iteritems()])
        )
